...
def build_main_prompt(result_type: str, result_level: str, result_metadata: dict, evidence_context: str = "") -> str:
    """
    Build prompt for generating new_title, new_description, and short_name (if applicable).
    
    Args:
        result_type: Type of result (e.g., "innovation development")
        result_level: Level of result (e.g., "output", "outcome")
...
"""
    ...
    # Add short name instructions for Innovation Development
...
## Important instructions for results at the Output level:
...
## Additional considerations for rewriting the title:
...
"""
        ...
        additional_level_instruction = """
## Important instructions for results at the Outcome level:
...
"""
    ...
    prompt += f"\n## Context\n\n### Result Metadata\nUse this metadata as the foundation for your suggestions:\n```json\n{context}\n```\n"
    
    # Add evidence context if available
    if evidence_context:
        prompt += f"\n### Evidence Sources\nUse the following evidence to validate and enhance your response:\n\n{evidence_context}\n\n**Important**: Include specific data, findings, or outcomes mentioned in the evidence sources. Ensure consistency between your response and the evidence content.\n"
