...
@router.post(
    "/api/feedback/services/register",
    response_model=Dict[str, Any],
    tags=["Services"],
    summary="Register a new AI service",
    description="""
    🏗️ **Manual AI Service Registration**
    
    Register a new AI service for feedback collection with detailed metadata.
    This allows for better analytics and service-specific insights.
    
    **Registration Benefits:**
    - Enhanced service analytics with proper naming
    - Context field tracking for service-specific insights
    - Better organization in feedback summaries
    - Custom service descriptions for reporting
    ...
    **Example Registration:**
    ```json
    {
        ...
        "description": "AI-powered report generation for AICCRA data analysis",
        "expected_context": ["report_type", "data_sources", "template_version"]
    }
    ```
    
    **Note:** Services are automatically registered when first encountered,
    but manual registration allows for better metadata and analytics.
    """
)
async def register_ai_service(
    service_name: str = Field(..., description="Unique service identifier (lowercase, hyphens allowed)"),
    ...
    expected_context: Optional[List[str]] = Field(
        default=None, 
        description="Expected context fields for this service",
        examples=[["report_type", "data_sources"], ["analysis_type", "model_version"]]
    )
) -> Dict[str, Any]:
    """Register a new AI service for feedback collection."""
    try:
        logger.info(f"📝 Registering new AI service: {service_name}")
        logger.info(f"🏷️ Display name: {display_name}")
        logger.info(f"📋 Expected context: {expected_context or 'None specified'}")
        ...
        if not service_name or not service_name.replace("-", "").replace("_", "").isalnum():
            raise HTTPException(
                ...
                detail={
                    ...
                    "details": "Service name must contain only letters, numbers, hyphens, and underscores",
                    "status": "error"
                }
            )
        
        existing_services = ai_feedback_service.get_registered_services()
        if service_name in existing_services:
            logger.info(f"🔄 Service {service_name} already exists, updating registration")
        
        success = ai_feedback_service.register_service(
            service_name=service_name,
        ...
        if success:
            ...
            return {
                ...
                "integration_info": {
                    ...
                    "service_analytics": f"/api/feedback/service/{service_name}",
                    "usage_example": {
                        "service_name": service_name,
                        "user_id": "user@example.com",
                        "user_input": "Example user input",
                        "ai_output": "Example AI response",
                        "feedback_type": "positive",
                        "context": {field: "example_value" for field in (expected_context or [])}
                    }
                }
            }
        else:
            raise HTTPException(
                ...
                detail={
                    ...
                    "details": "Service registration failed due to internal error",
                    "status": "error"
                }
            )
        ...
        raise HTTPException(
            ...
            detail={
                "error": "Internal server error",
                "details": f"Failed to register service: {str(e)}",
                "status": "error"
            }
        )