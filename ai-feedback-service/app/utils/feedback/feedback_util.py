...
class AIInteractionService:
    ...
    def _get_service_info(self, service_name: str, display_name: Optional[str] = None, service_description: Optional[str] = None) -> Dict[str, Any]:
        """Get service information, prioritize request data over registered services."""
        
        # ALWAYS use provided display_name and service_description if available
        if display_name and service_description:
            logger.info(f"🎯 Using provided metadata for service: {service_name}")
            return {
                "name": display_name,
                "description": service_description
            }
        elif display_name:
            logger.info(f"🎯 Using provided display name for service: {service_name}")
            return {
                "name": display_name,
                "description": service_description or f"AI service: {service_name}"
            }
        elif service_description:
            logger.info(f"🎯 Using provided description for service: {service_name}")
            return {
                "name": display_name or service_name.replace("-", " ").replace("_", " ").title(),
                "description": service_description
            }
        
        # Fallback to registered service only if no data provided
        if service_name in self.registered_services:
            logger.info(f"📋 Using registered service info for: {service_name}")
            return self.registered_services[service_name]
        else:
            # Auto-register with generic info
