...
export interface ColumnDef {
  key: string;
  label: string;
  type: 'checkbox' | 'text' | 'number' | 'textarea' | 'select' | 'status' | 'link' | 'chips' | 'partners' | 'sdg' | 'evidence_desc' | 'evidence_link' | 'training_purpose' | 'date' | 'countries' | 'regions' | 'affiliation' | 'nationality';
  readonly?: boolean;
  required?: boolean;
  options?: string[];
  tooltip?: string;
